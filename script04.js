...
/* --------------------------
   Estoque (Tabela 3) - create row & logic
   --------------------------*/
function createEstoqueRow(data) {
  ...
  // 2: DISCRIMINAÇÃO (select). When changed update unit display. Provide "Outro" option to add new item.
  ...
  // unit span
  const spanUnit = document.createElement('span'); spanUnit.style.marginLeft='6px'; spanUnit.style.fontWeight='bold';
  ...
  // 5: SALDO ATUAL (input) - per spec user-provided; compute cons = quant + estAntes - saldo
  ...
  const tdRel = document.createElement('td'); const pRel = document.createElement('p'); pRel.style.cursor='pointer'; pRel.textContent = data?.rel ?? '';
  ...
  sel.addEventListener('change', () => {
    if (sel.value === '__OUTRO__') {
      ...
      const div = document.createElement('div'); div.style.display='flex'; div.style.gap='4px'; div.style.marginTop='4px';
      ...
      ok.addEventListener('click', () => {
        ...
        sel.value = nm;
        updateUnit();
        div.remove(); tdDisc._outroShown = false;
      });
      cancel.addEventListener('click', () => { div.remove(); tdDisc._outroShown = false; sel.value = ''; updateUnit(); });
      div.appendChild(nameIn); div.appendChild(unitIn); div.appendChild(ok); div.appendChild(cancel); tdDisc.appendChild(div);
      tdDisc._outroShown = true;
      return;
    } else {
      updateUnit();
      // inform user of unit change
      const u = spanUnit.textContent || '(não definido)';
      alert('Unidade do item selecionado: ' + u);
    }
  });
