...
const orientacaoMerenda = `Este formulário deve ser preenchido com as informações do dia a dia do serviço de merenda, registrando a data em que foi servida a merenda, a quantidade de alunos que foram atendidos e o cardápio servido no dia. Informar ainda se houve substituição ou não, qual foi a substituição, qual o motivo da substituição e, na tabela abaixo, informar o consumo de alimentos, a quantidade de alimentos e itens recebidos, a quantidade que já havia em estoque, quanto foi utilizado e se há sobra no momento da entrega do mapa de merenda escolar.`;
...
function createCardapioRow(dateObj, periodStart, periodEnd) {
  ...
  pCard.addEventListener('click', () => {
    ...
    select.addEventListener('change', () => {
      ...
      if (novo !== antigo) {
        ...
        registrarRelatorio(
          pData.textContent,
          `Cardápio substituído: "${antigo}" por "${novo}"`
        );
      }
      select.remove();
    });
  ...
  function registrarRelatorio(data, texto){
    const tabela = document.getElementById("relatorios_body");
    if(!tabela) return;
  ...
  // col7 justificativa (p -> textarea)
  // col7 justificativa (texto fixo de orientação)
  const tdJust = document.createElement('td');
  tdJust.className = 'justificativa-cell';

  const pJust = document.createElement('p');
  pJust.textContent = orientacaoMerenda;
  pJust.style.whiteSpace = "normal";
  pJust.style.fontSize = "12px";
...
function exportAll() {
  ...
  rows.forEach(tr => {
    ...
    payload.cardapio.push({
      ...
      justificativa: tds[6]?.querySelector('p')?.textContent || ''
    });
  });
  // estoque
  payload.estoque = [];
    ...
    if (Array.isArray(data.cardapio)) {
      data.cardapio.forEach(item => {
        ...
        if (last) {
          ...
          if (tds[6]?.querySelector('p')) tds[6].querySelector('p').textContent = item.justificativa || '';
        }
      });
    }
...
function registrarRelatorio(data, texto) {
  const body = el.estoqueBody();
  if (!body) return;

  const dataFormatada = formatDateBR(data);

  const linhas = Array.from(body.children);

  linhas.forEach(tr => {
    const colRelatorio = tr.children[5]; // última coluna da tabela 2
    if (!colRelatorio) return;

    const registro = `${dataFormatada} - ${texto}`;

    if (colRelatorio.textContent.trim() === "") {
      colRelatorio.textContent = registro;
    } else {
      colRelatorio.textContent += "\n" + registro;
    }
  });
}
function registrarSubstituicao(data, original, novo, motivo) {

  const texto = `Substituição de cardápio: ${original} → ${novo}. Motivo: ${motivo}`;

  registrarRelatorio(data, texto);

}
if (isSaturday(data)) {

  const texto = "Sábado letivo";

  registrarRelatorio(data, texto);

}
registrarRelatorio(data, "Sábado letivo - reposição de aula");
