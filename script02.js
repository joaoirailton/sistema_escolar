/* script.js - DEMONSTRATIVO MESTRAL MESTRE PACÍFICO - DINÂMICO V1
   Funcionalidades:
   - Sincroniza sidebar -> Tabela Detalhes (nome, endereço, meio, município, nº alunos, tipo ensino, período)
   - Valida período inicial <= final
   - Gera Tabela 2 (cardápio) com 1 linha por dia letivo (segunda..sábado)
   - Numera Quant. Dias automaticamente
   - Permite editar data da linha dentro do período (proíbe domingos)
   - Frequências (Creche, Ed.Inf/Fund, EJA) aparecem como "X" e ao clicar viram input number; ao blur voltam a <p>
   - Verifica soma das três frequências vs total de alunos e alerta/pinta a linha
   - Cardápio por semana (regular/integral) com repetição na 5ª semana; clicar no cardápio abre select com opções
   - Ao trocar cardápio, insere justificativa automática "O cardápio era "...", foi mudado por "..."
   - Justificativa automática para sábados: "Sábado letivo, foi servida merenda" (editável)
   - Botão adicionar/remover linhas em ambas as tabelas
   - Tabela estoque: select de discriminação com lista, item "Outro" cria campo para nome + unidade; unidade exibida; saldo/consumos calculados
   - Export / Import JSON (tudo: detalhes, cardápio, estoque)
   - Imprimir: oculta sidebar automaticamente e restaura após impressão
*/
...
function weekOfMonth(date) {
  return Math.floor((date.getDate() - 1) / 7) + 1; // 1..5
}
...
function getCardapioForDate(d, tipo) {
  ...
  const idx = w > 4 ? 1 : w;
  const day = d.getDay(); // 1..6 (segunda..sábado), 0 domingo
  if (day === 0) return "";
...
function checkRowSum(tr) {
  ...
  if (total > 0 && sum > total) {
    ...
    alert(`A soma das frequências (${sum}) excede o número total de alunos (${total}).`);
  } else {
...
function createCardapioRow(dateObj, periodStart, periodEnd) {
  ...
  pCard.addEventListener('click', () => {
    ...
    const curOpt = document.createElement('option'); curOpt.value = pCard.textContent; curOpt.textContent = pCard.textContent; select.appendChild(curOpt);
    getCardapioOptions(tipo).forEach(optText => {
  ...
  // attach checkRow when any frequency value changes (global delegation not used here)
  ['click','input','blur'].forEach(evt => {
...
/* --------------------------
   Estoque (Tabela 3) - create row
   --------------------------*/
function createEstoqueRow(data) {
  // data optional: {quant, discr, cons, estAntes, saldo, rel}
  const tbody = el.estoqueBody();
  if (!tbody) return;
  const tr = document.createElement('tr');

  // 1: QUANT. (recebida)
  const tdQuant = document.createElement('td'); const inputQuant = document.createElement('input'); inputQuant.type='number'; inputQuant.min=0; inputQuant.value = data?.quant ?? ''; tdQuant.appendChild(inputQuant); tr.appendChild(tdQuant);

  // 2: DISCRIMINAÇÃO (select). When clicked/changed update unit display. Provide "Outro" option to add new item.
  ...
  if (data?.discr) {
    const found = Array.from(sel.options).find(o => o.value === data.discr);
    if (found) sel.value = data.discr;
    else {
      // add new dynamic item
      estoqueItems.push({name:data.discr, unit: data.unit || ''});
      sel.appendChild(new Option(data.discr,data.discr));
      sel.value = data.discr;
    }
  }
  tdDisc.appendChild(sel);
  // unit span
  const spanUnit = document.createElement('span'); spanUnit.style.marginLeft='6px'; tdDisc.appendChild(spanUnit);
  tr.appendChild(tdDisc);

  // 3: QUANT. DE ALIM. CONS. (computed readOnly)
  const tdCons = document.createElement('td'); const inputCons = document.createElement('input'); inputCons.type='number'; inputCons.readOnly = true; inputCons.value = data?.cons ?? ''; tdCons.appendChild(inputCons); tr.appendChild(tdCons);

  // 4: QUANT. DE ALIM. JÁ EM ESTOQUE (antes)
  const tdEstAntes = document.createElement('td'); const inputEstAntes = document.createElement('input'); inputEstAntes.type='number'; inputEstAntes.min=0; inputEstAntes.value = data?.estAntes ?? ''; tdEstAntes.appendChild(inputEstAntes); tr.appendChild(tdEstAntes);

  // 5: SALDO ATUAL (input) - per spec this is user-provided and used to compute cons = quant + estAntes - saldo
  const tdSaldo = document.createElement('td'); const inputSaldo = document.createElement('input'); inputSaldo.type='number'; inputSaldo.min=0; inputSaldo.value = data?.saldo ?? ''; tdSaldo.appendChild(inputSaldo); tr.appendChild(tdSaldo);

  // 6: RELATÓRIO (p -> textarea on click)
  const tdRel = document.createElement('td'); const pRel = document.createElement('p'); pRel.style.cursor='pointer'; pRel.textContent = data?.rel ?? ''; pRel.addEventListener('click', () => {
    if (tdRel.querySelector('textarea')) return;
    const ta = document.createElement('textarea'); ta.className='edit-field'; ta.value = pRel.textContent;
    ta.addEventListener('blur', () => { pRel.textContent = ta.value; ta.remove(); if (!tdRel.contains(pRel)) tdRel.appendChild(pRel); });
    tdRel.appendChild(ta); ta.focus();
  }); tdRel.appendChild(pRel); tr.appendChild(tdRel);

  // 7: botão remover
  const tdBtn = document.createElement('td'); const btnRm = document.createElement('button'); btnRm.className='remove-row-btn'; btnRm.type='button'; btnRm.textContent='🗑'; btnRm.addEventListener('click', () => tr.remove()); tdBtn.appendChild(btnRm); tr.appendChild(tdBtn);

  // update unit display
  function updateUnit() {
    const v = sel.value;
    if (!v || v === '__OUTRO__') { spanUnit.textContent = ''; return; }
    const found = estoqueItems.find(it => it.name === v);
    spanUnit.textContent = found ? found.unit : '';
  }
  sel.addEventListener('change', () => {
    if (sel.value === '__OUTRO__') {
      // show inputs to add new
      const div = document.createElement('div'); div.style.display='flex'; div.style.gap='4px';
      const nameIn = document.createElement('input'); nameIn.type='text'; nameIn.placeholder='Nome do item';
      const unitIn = document.createElement('input'); unitIn.type='text'; unitIn.placeholder='Unidade (ex: kg)';
      ...
      ok.addEventListener('click', () => {
        ...
        if (!nm) { alert('Informe nome do item'); return; }
        estoqueItems.push({name:nm, unit: uni});
        // rebuild select and set selected
        sel.innerHTML = '';
        sel.appendChild(new Option('-- selecione --',''));
        estoqueItems.forEach(it => sel.appendChild(new Option(it.name, it.name)));
        sel.appendChild(new Option('Outro','__OUTRO__'));
      ...
      div.appendChild(nameIn); div.appendChild(unitIn); div.appendChild(ok); tdDisc.appendChild(div);
      return;
    } else {
      updateUnit();
      // inform user of unit change
      alert('Unidade do item selecionado: ' + (spanUnit.textContent || '(não definido)'));
    }
  });
  // compute cons = quant + estAntes - saldo
  function computeCons() {
    ...
    const cons = q + e - s;
    inputCons.value = cons;
  }
  [inputQuant, inputEstAntes, inputSaldo].forEach(inp => inp.addEventListener('input', computeCons));
  // initialize display
  updateUnit();
  computeCons();
